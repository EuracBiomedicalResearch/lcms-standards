...
std_serum_files <- data.frame(
  folder <- "2020/2020_01",
  mzML   <- list.files("Z:/mzML/2020/2020_01"))
...
```{r, eval=FALSE}
raw_data <- readMSData(files = paste0("Z:/mzML/", std_serum_files$folder, 
                                      "/", std_serum_files$mzML), 
                       mode = "onDisk")
std_serum_files$time[1] <- as.character(extract_time_stamp(fileNames(raw_data[1])))
