...
for(i in seq(nrow(std_info))){
  ...
  if(length(unlist(which_within(mz.i, feat_filter$mzmed))) == 1){
    ...
    feat_filter$annotation[idx] <- std_info[i, grep(polarity, colnames(std_info))]
    idy <- which(feat_filter$cmp == feat_filter$cmp[idx])
    idy.c <- feat_filter$compound[idy]
    idy.c <- idy.c[grep("^[X].*", idy.c)]
    idy.c <- idy.c[!duplicated(idy.c)]
    for(j in length(idy.c)){
      idz <- which(feat_filter$compound == idy.c[j] & feat_filter$cmp == feat_filter$cmp[idx])
      feat_filter$compound[idz] <- ih_cmp$comments[ih_cmp$cmp_ID == idy.c[j]]
    }
    
  } else if(length(unlist(which_within(mz.i, feat_filter$mzmed))) > 1) {
    idx <- unlist(which_within(mz.i, feat_filter$mzmed))
    feat.i <- feat_filter[idx,]
    idx <- idx[unlist(which_within(rt.i, feat.i$rtmed, mzd = 20))]
