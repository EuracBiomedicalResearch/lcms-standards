...
In a compound with $n_X$ atoms of element $X$ it is possible that 
${n_X}_1$, ${n_X}_2$, ..., ${n_X}_q$ (${n_X}_1+{n_X}_2+ ... +{n_X}_q = n_X$) 
atoms of $X$ have isotope form $1, 2, ... q$. Each configuration 
$C_X = ({n_X}_1, {n_X}_2, ..., {n_X}_q)$ will result in a different shift in 
mass $md_{C_X}$. 
The probability $P(C_X)$ of observing such a configuration con be computed using 
the multinomial distribution.
The overall mass difference $md$ with respect to the monoisotopic mass is 
determined by the configurations of each element $X, Y,... Z$ in the molecule 
i.e. by $C = (C_X, C_Y... C_Z)$ and is given by the sum 
$md = md_{C_X}+ md_{C_Y} ... +md_{C_Z}$. Finally the probability of 
observing the overall configuration $C$ (and the corresponding mass difference 
$md$) is $P(C) = P(md) = P(C_X)* P(C_Y)...*P(C_Z)$ i.e. the product of the 
probability of observing the configurations related to each element.

There is a very high number $N_{C_X} = \binom{q + n_X -1}{n_X}$ of possible 
configurations $C_X$ for the atoms of $X$ and a even greater 
number $N_{C_X}* N_{C_Y}...*N_{C_Z}$ of overall configurations 
$C=(C_X, C_Y..., C_Z)$. Here we keep only those that are significant for our 
purpose.
...
## Selecting "significant" families of configurations

Here we try to determine which families of configurations (or equivalently which 
mass differences with respect to the monoisotopic mass) to keep.

A first idea could be to compute the isotope pattern for each element in HMDB  
and store the configurations (more precisely, the families associated that 
configuration) that are related to isotope pattern peaks with intensity higher 
than a certain threshold (whose value we have to decide). Then we could decide 
to keep the most frequent ones (and we would have to decide a certain cutoff).
...
```{r, eval = !file.exists(paste0("data/RData/isotope-intensity-estimation/"))}
data(isotopes)
chemforms <- cmps$formula
threshold = 0.01
...
cnf_per_cmpd <- lapply(iso_ps, function(iso_p) {
  tmp <- iso_p[, intersect(colnames(iso_p), c("13C", "2H", "15N", "17O", "18O", 
                                            "33S", "34S", "35S", "36S", 
                                            "36Cl", "37Cl")), drop = FALSE]
  apply(tmp, 1, 
        function(row) paste(sapply(names(row)[which(row > 0)], 
                                   function(nm) paste0(row[nm],"[", nm, "]")),
                            collapse = ""))
})
cnf_frequency <- table(do.call(c, cnf_per_cmpd))
...
```{r, echo = FALSE, eval = file.exists(paste0("data/RData/isotope-intensity-estimation/"))}
load(paste0(dr, "cnf_frequencies.RData"))
```


```{r}
cnf_proportion <- sort(cnf_frequency / nrow(cmps), decreasing = TRUE)[-1]
par(mar = c(8, 4.1, 4.1, 2.1))
barplot(cnf_proportion[cnf_proportion > 0.5], las = 2)
```

In the plot above we can see the families of configurations that for compounds 
in HMBD produce a significant peak for at least a half of the compounds
(the height of the bar specifies the exact proportion). 
In other words, if we randomly sample a compund in HMBD the chance to observe a 
significant peak related to the above families will be high. I observe however 
that this approach penalizes families related to elements with low counts in 
HMBD compounds. The number of times a family related to N, S or Cl is found to 
be significant is limited by the fact that the number of compounds in HMBD that 
contain these elements is much lower than those that contain C, H, or O.  
