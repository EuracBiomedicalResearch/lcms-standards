...
isotopologues that would result in a peak with absolute probability above a
certain threshold. We next identify the most frequently observed isotopologues
(across all compounds in HMDB). For each of these isotopologues we then
determine their mass difference and evaluate the observed intensity
(probability) ratios between the monoisotopic peak and the isotopologue of all
compounds in HMDB for which the isotopologue was reported. While the mass
...
compound. For each isotopologue we have however the empirically determined
...
consisting of a larger number of atoms. To evaluate that we plot below the
counts of individual elements against the mass of HMDB compounds.
