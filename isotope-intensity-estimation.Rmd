...
res <- lapply(seq_len(nrow(isotopes)), function(i) {
    niso <- isotopes$number_isotopes[i]
    sapply(counts_df[, isotopes$element[i]], function(n)
        ifelse(n > 0, dmultinom(c(n - niso, niso),
                                prob = c(isotopes$element_abundance[i],
                                         isotopes$isotope_abundance[i])), 0))
})
prob_df <- do.call(cbind, res)
colnames(prob_df) <- isotopes$isotope

## ??? Is the above correct???

prob_df <- data.frame(
    `C13` = sapply(counts_df$C, function(n)
        ifelse(n > 0, dmultinom(c(n - 1, 1), prob = isotopes$C), 0)),
    `2C13` = sapply(counts_df$C, function(n)
        ifelse(n > 1, dmultinom(c(n - 2, 2), prob = isotopes$C), 0)),
    `N15` = sapply(counts_df$N, function(n)
        ifelse(n > 0, dmultinom(c(n - 1, 1), prob = isotopes$N), 0)),
    `O17` = sapply(counts_df$O, function(n)
        ifelse(n > 0, dmultinom(c(n - 1, 1, 0), prob = isotopes$O), 0)),
    `O18` = sapply(counts_df$O, function(n)
        ifelse(n > 0, dmultinom(c(n - 1, 0, 1), prob = isotopes$O), 0)),
    `S33` = sapply(counts_df$S, function(n)
        ifelse(n > 0, dmultinom(c(n - 1, 1, 0), prob = isotopes$S), 0)),
    `S34` = sapply(counts_df$S, function(n)
        ifelse(n > 0, dmultinom(c(n - 1, 0, 1), prob = isotopes$S), 0)),
    `Cl37` = sapply(counts_df$Cl, function(n)
        ifelse(n > 0, dmultinom(c(n - 1, 1), prob = isotopes$Cl), 0)),
    check.names = FALSE)
```
...
```{r}
prob_limits <- data.frame(
    `C13` = sapply(count_limits[, "C"], function(n)
        ifelse(n > 0, dmultinom(c(n - 1, 1), prob = isotopes$C), 0)),
    `2C13` = sapply(count_limits[, "C"], function(n)
        ifelse(n > 1, dmultinom(c(n - 2, 2), prob = isotopes$C), 0)),
    `N15` = sapply(count_limits[, "N"], function(n)
        ifelse(n > 0, dmultinom(c(n - 1, 1), prob = isotopes$N), 0)),
    `O17` = sapply(count_limits[, "O"], function(n)
        ifelse(n > 0, dmultinom(c(n - 1, 1, 0), prob = isotopes$O), 0)),
    `O18` = sapply(count_limits[, "O"], function(n)
        ifelse(n > 0, dmultinom(c(n - 1, 0, 1), prob = isotopes$O), 0)),
    `S33` = sapply(count_limits[, "S"], function(n)
        ifelse(n > 0, dmultinom(c(n - 1, 1, 0), prob = isotopes$S), 0)),
    `S34` = sapply(count_limits[, "S"], function(n)
        ifelse(n > 0, dmultinom(c(n - 1, 0, 1), prob = isotopes$S), 0)),
    `Cl37` = sapply(count_limits[, "Cl"], function(n)
        ifelse(n > 0, dmultinom(c(n - 1, 1), prob = isotopes$Cl), 0)),
    check.names = FALSE)
```
...
par(mfrow = c(ncol(prob_df), 1), mar = 4, 4, 1, 0.5)
...
- [ ] Should we consider to lower the count for C13?
- [ ] For Cl37, should we increase the upper count?
- [ ] Should we use the `range` instead of the quantiles?
- [ ] Maybe better to define a `function` to calculate the probabilities instead
      of doing it manually for each isotope.
