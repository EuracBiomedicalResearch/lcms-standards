...
# should rel_to = 2 be used above? I would say that the most correct thing is to
...
We thus defined `r length(subst_frequency)` isotopologues considering all
...
```{r}
intens <- unlist(lapply(iso_ps, function(iso_p) iso_p[,"abundance"][-1]),
                      use.names = FALSE)
...
ggplotly(p + geom_vline(xintercept = 0.5, color = "red", size= .1))
```
Here I compute the ratios R directly from enviPat::isopattern output.
```{r}
Rs <- lapply(iso_ps, function(iso_p) iso_p[-1, "abundance"]/iso_p[1, "abundance"])
selected_substs <- names(subst_proportion_var2[subst_proportion_var2 > 0.5])
temp <- lapply(chemforms, function (frml){
  Rs[[frml]][match(selected_substs, subst_per_cmpd[[frml]])]
})
R_df <- do.call(rbind, temp)
colnames(R_df) <- selected_substs
```

and I plot those related to the selected substitutions against the mass of the compounds. The plots of R when 
only a single element appears in the configuration with multiplicity 1 
(e.g. 1[13C], 1[15N]) are very similar to the plot of the counts vs mass 
(in that case R is equal to a constant times the counts). However, for some 
other configuration the relationship is quadratic or cubic etc. The same linear 
regression or linear bounds maybe wouldn’t be very accurate in every case but we 
can see how the approximation works.
...
```{r}
selected_names <- names(subst_proportion_var2[subst_proportion_var2 > 0.5])
