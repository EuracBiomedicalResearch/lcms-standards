...
```{r, table-all-ms2-hmdb_MIX12, echo = FALSE, results = "asis"}
...
```{r mix12-serum-pos-7-Aminoheptanoic acid-ms2, echo = FALSE}
...
- FT00972 (RT = 169.5, 9.554 ppm, `[M+2Na]2+` ion): with confidence **D**? 
  Mixture only contains 4 stds, so maybe high ppm during run? FINDME
...
```{r mix12-serum-pos-3-Chloro-5-(trifluoromethyl)-2-pyrindol-ms2, echo = FALSE}
plot_spectra(std_ms2)
```
...
```{r mix12-serum-pos-"3,5-Dichloro-2-hydroxypyridine"-ms2, echo = FALSE}
plot_spectra(std_ms2)
```
...
```{r mix12-serum-pos-"3,5,6-Trichloro-2-pyrinidol"-ms2, echo = FALSE}
plot_spectra(std_ms2)
```
...
The filtered feature table for `r std` is empty, so change code chunk below in 
able to render document possible.

```{r, table-feature-matches_MIX20, results = "asis", echo = FALSE, message = FALSE}
...
```{r mix12-serum-neg-"3-Chloro-5-(trifluoromethyl)-2-pyrindol"-ms2, echo = FALSE}
plot_spectra(std_ms2)
```
...
- FT0839 (RT = 38.5, 19.8 ppm, `[M-H]-` ion): nice peak but high ppm so 
  probably other compound or **D**? Mixture only contains 4 stds, so maybe high 
  ppm during run? FINDME
...
```{r mix12-serum-neg-"3,5-Dichloro-2-hydroxypyridine"-ms2, echo = FALSE}
plot_spectra(std_ms2)
```
...
- No MS2 reference spectra available in HMDB and MassBank.
- No MS2 spectra available.
- FT0464 (RT = 38.62, 21.26 ppm, `[M-H]-` ion) with confidence **D**? Mixture 
  only contains 4 stds, so maybe high ppm during run? FINDME
- FT1009 (RT = 35.93, 19.85 ppm, `[M+CHO2]-` ion) with confidence **D**? Mixture 
  only contains 4 stds, so maybe high ppm during run? FINDME
...
```{r mix12-serum-neg-"3,5,6-Trichloro-2-pyrinidol"-ms2, echo = FALSE}
plot_spectra(std_ms2)
```
...
- No MS2 reference spectra available in HMDB and MassBank.
- FT0832 (RT = 31.13, 19.82 ppm, `[M-H]-` ion) with confidence **D**? Mixture 
  only contains 4 stds, so maybe high ppm during run? FINDME
