...
mix.n <- 18       # if study == MIX, specify which MIX number
da <- 0.01
...
std_info$cmpname <- tolower(gsub(" .*", "", std_info$name))
std_info$cmpname <- gsub("l-", "", std_info$cmpname)
std_info$cmpname <- gsub("alpha-", "", std_info$cmpname)
std_info$cmpname <- gsub("Gamma-", "", std_info$cmpname)
for(i in seq(nrow(std_info))){
  if((substr(std_info$cmpname[i], 1, 1) %in% (seq(9))) & 
     (substr(std_info$cmpname[i], 3, 3) %in% letters)){
    std_info$cmpname[i] <- paste(gsub("*.-", "", std_info$cmpname[i]), 
                                 gsub("-.*", "", std_info$cmpname[i]), sep = "_")
  } else if((substr(std_info$cmpname[i], 1, 1) %in% (seq(9))) & 
            (substr(std_info$cmpname[i], 3, 3) %in% seq(9))){
    std_info$cmpname[i] <- paste(gsub(".*-", "", std_info$cmpname[i]), 
                                 gsub(",", "_", gsub("-.*", "", std_info$cmpname[i])), 
                                 sep = "_")
  }
}
std_info$cmpname <- gsub("-", "_", std_info$cmpname)
std_info$cmpname[std_info$name == "Glucose 1-Phosphate"] <- "glucose_phosphate_1"
std_info$cmpname[std_info$name == "Glucose 6-Phosphate"] <- "glucose_phosphate_6"
std_info$cmpname[std_info$name == "Fructose-1,6-Diphosphate"] <- "fructose_diphosphate"
std_info$cmpname[std_info$name == "Ribose 5-Phosphate"] <- "ribose_phosphate_5"
std_info$cmpname[std_info$name == "Ribulose 5-Phosphate"] <- "ribulose_phosphate_5"
std_info$cmpname[std_info$name == "Glutathione Oxidized"] <- "glutathione_oxidized"
std_info$cmpname[std_info$name == "Glutathione Reduced"] <- "glutathione_reduced"
std_info$cmpname[std_info$name == "C3 Carnitine"] <- "carnitine_c3"
std_info$cmpname[std_info$name == "C4 Carnitine"] <- "carnitine_c4"
std_info$cmpname[std_info$name == "C5 Carnitine"] <- "carnitine_c5"
std_info$cmpname[std_info$name == "5,6-Dihydro Thymine"] <- "dihydrothymine"
