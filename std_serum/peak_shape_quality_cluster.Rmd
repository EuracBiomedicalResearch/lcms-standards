  ...
  for(i in 1:(length(mix.levels)) #####################################
    ...
    for(k in 1:(nrow(std_info.i)) ######################################
      ...
      if(nrow(pks)){
        ...
        peak_quality$RT_delta[idx] <-round(max(pks3$rt) - min(pks3$rt))
        pks3$peak_width <- pks3$rtmax - pks3$rtmin
        peak_quality$peak_with_min[idx] <- round(min(pks3$peak_width))
        peak_quality$peak_with_max[idx] <- round(max(pks3$peak_width))
      } else{
