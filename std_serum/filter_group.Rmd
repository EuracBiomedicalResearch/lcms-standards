...
```{r}
data_NONimp_smpl <- data_NONimp[xdata$blank == 0, ]
feat$int <- apply(data_NONimp_smpl, 2, function(x){
  median(x, na.rm = TRUE)
})
data_NONimp_filter <- 
...
```{r}
tmp <- aggregate(data_NONimp, by = list(xdata$class), stat.cv)
tmp <- setNames(data.frame(t(tmp[,-1])), tmp[,1])
tmp <- tmp[,-1] # delete CV_blanks
feat$cv_intra <- NA
