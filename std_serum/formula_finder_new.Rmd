...
for(y in 1:length(mysamples)){
  ...
  formula_finder <- matrix(nrow=nrow(std_info), ncol = 6)
  rownames(formula_finder) <- std_info$abbreviation
  colnames(formula_finder) <- c("start", "adduct", "thr_filters", 
  ...
  for(z in 1:nrow(std_info)){
    ...
    if(!is.na(tmp$mz1[z])){
      ...
      myform <- update_form(H_rule(update_form(myform, myadd, "remove")), myadd, "add")
      myform <- update_form(N_rule(mymzneut, update_form(myform, myadd, "remove")), myadd, "add")
      myform <- update_form(RPU_rule(update_form(myform, myadd, "remove")), myadd, "add")
...
formfind <-  matrix(nrow=nrow(std_info), ncol = 6)
rownames(formfind) <- std_info$abbreviation
colnames(formfind) <- c("start", "adduct", "thr_filters", "A1_filter", 
                        "A2_filter", "rank")
