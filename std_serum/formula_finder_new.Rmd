...
for(y in 1:length(mysamples)){
  ...
  colnames(formula_finder) <- c("start", "adduct", "H_rule", "H_rule2", 
                                "N_rule", "RPU_rule", 
  ...
  for(z in 1:nrow(std_info)){
    ...
    if(!is.na(tmp$mz1[z])){
      ...
      if(length(myform) > 0){
        ...
        if(length(H_rule2(mymzneut, 
                          update_form(myform, myadd, "remove"))) > 0){
          myform <- update_form(H_rule2(mymzneut, update_form(
            myform, myadd, "remove")), myadd, "add")
          formula_finder[z, "H_rule2"] <- length(myform)
          if(length(N_rule(
            mymzneut, update_form(myform, myadd, "remove"))) > 0){
            if(mymzneut < 500){
            ...
            if(length(myform) > 0){ 
              ...
              if(length(myform) > 0){
                ...
                if(any(myform$rank[myform$formula == myformion])){
                  formula_finder[z, "rank"] <- 
                    myform$rank[myform$formula == myformion]
                } else {
                  formula_finder[z, "rank"] <- 9999
                }
              } # end if length(myform) > 0 after isotope_rule 2
            } # end if length(myform) > 0 after isotope_rule 1
          } # end if length N_rule > 0
        } # end if(any) H_rule2()
      } # end if 'length(myform) > 0' after getform()
...
formfind <-  matrix(nrow=nrow(std_info), ncol = 14)
rownames(formfind) <- std_info$abbreviation
colnames(formfind) <- c("formula", "POS", "NEG", 
                        "quality_NEG", "quality_POS", 
                        "start", "adduct", "H_rule", "H_rule2", 
                        "N_rule", "RPU_rule", 
                        "A1_filter", "A2_filter", "rank")
formfind[, "formula"] <- std_info$formula
formfind[, "POS"] <- std_info$POS
formfind[, "quality_POS"] <- std_info$quality_POS
formfind[, "NEG"] <- std_info$NEG
formfind[, "quality_NEG"] <- std_info$quality_NEG

for(i in 1:nrow(formfind)){
  for(j in 6:ncol(formfind)){
    tmp <- c()
    for(k in 1:length(formula_finder)){
      tmp <- c(tmp, formula_finder[[k]][i,(j-5)])
    }
    tmp <- tmp[!is.na(tmp)]
    if(is.na(tmp[1])){
      formfind[i, j] <- NA
