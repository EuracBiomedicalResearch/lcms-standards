...
std_info <- std_info[-idx, ]
std_info$formula[std_info$abbreviation == "chlorotrifluoromethylpyrindol"] <- "C6H3NOClF3"
std_info$formula[std_info$abbreviation == "dichlorohydroxypyridine"] <- "C5H3NOCl2"
std_info$formula[std_info$abbreviation == "trichloropyrinidol"] <- "C5H2NOCl3"
...
for(z in 1:nrow(std_info)
  ...
  if(!is.na(tmp$mz1[z])){
    ...
    if(grepl("Cl", std_info$formula[z])){
      tmp.frm1 <- data.frame(
        formula = decomposeMass(tmp$mz1[z], ppm = myppm,
      ...
      tmp.frm1 <- data.frame(
        formula = decomposeMass(tmp$mz1[z], ppm = myppm)$formula) 
    }
    tmp.frm1 <- cbind(
      tmp.frm1, 
    ...
    if(nrow(tmpx) > 0){
      ...
      if(!is.na(tmp$i3[z])){tmpx$A2x <- abs(tmpx$A2 - tmp.A2)} else {tmpx$A2x <- 0}
      tmpx$A <- NA
      #tmpx$Aw <- NA
      ...
      for(i in 1:nrow(tmpx)){
        ...
        if(!is.na(tmp$i3[z])){tmpx$A2p[i] <- abs(100 - ((tmp.A2*100)/tmpx$A2[i]))} else {tmpx$A2p[i] <- 0}
