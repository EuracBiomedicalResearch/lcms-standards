  ...
  for(z in 1:nrow(std_info)
    ...
    if(!is.na(tmp$mz1[z])){
      ...
      if(!is.na(tmp$i2[z])){
        ...
        tmp.frm1$C_thr_min <- 
          tmp.frm1$C_thr - (tmp.frm1$C_thr * myerror_A1)
        tmp.frm1$C_thr_max <- 
          tmp.frm1$C_thr + (tmp.frm1$C_thr * myerror_A1)
        tmp.frm1$C_rule <- 
      ...
      if(nrow(tmpx) > 0){
        ...
        if(any(which(tmpx$fml == std_info$formula[z]))){
          tmpx <- tmpx[order(tmpx$A), ]
          formula_finder[z, "rank"] <- 
            which(tmpx$fml == std_info$formula[z])
          tmpx <- tmpx[order(tmpx$Ap),]
          formula_finder[z, "rankp"] <- 
            which(tmpx$fml ==std_info$formula[z])
        } else {
          formula_finder[z, "rank"] <- 9999
          formula_finder[z, "rankp"] <- 9999
        }
