...
```{r, echo = FALSE}
fts <- data.frame(feature_id = c("FT02361", "FT02360"),
                  confidence_level = c("B", "B"))
## Get MS2 spectra:
ms2 <- std_ms2[match(c("FT02361_F07.S0595",
                       'FT02360_F07.S0595'),
...
```{r mix16-serum-pos-Acetylmethionine-ms2-hmdb, echo = FALSE, fig.width = 5, fig.height = 5}
hmdb_id <- std_dilution$HMDB[std_dilution$name == std]
std_hmdb <- Spectra(cdb, filter = ~ compound_id == hmdb_id)
sim_hmdb <- plot_ms2_similarity_heatmap(std_ms2, std_hmdb, csp)
```

```{r mix16-serum-pos-Acetylmethionine-ms2-mbank, echo = FALSE, fig.width = 5, fig.height = 5}
std_mbank <- get_mbank(mbank, inchikey = unique(std_hmdb$inchikey))
sim_mbank <- plot_ms2_similarity_heatmap(std_ms2, std_mbank, csp)
```
...
```{r mix16-serum-pos-Acetylmethionine-mirror-hmdb, echo = TRUE, fig.cap = "Mirror plots"}
std_ms2_sel <- plot_select_ms2(std_ms2, std_hmdb, 0.2, ppm = csp@ppm,
                               tolerance = csp@tolerance, sim = sim_hmdb)
```

```{r mix16-serum-pos-Acetylmethionine-mirror-mbank, echo = TRUE, fig.cap = "Mirror plots"}
...
- low similarity for FT02151 and FT02150, labels:

| label |   feature  | mzmed | ppm_error | rtmed | target_RT | feature_group |  adduct      | n_ms2 | mean_high | mean_low |
|:-----:|:----------:|:-----:|:---------:|:-----:|:---------:|:-------------:|:------------:|:-----:|:---------:|:--------:|
| **C** | FT02151    | 192.1 | 0.7651    | 37.22 | 37        | FG.001.001    | [M+H]+       | 1     | 18.48     | 16.73    |
| **C** | FT02150    | 192.1 | 1.141     | 44.79 | 37        | FG.002.001    | [M+H]+       | 2     | 16.86     | 13.51    |
...
```{r, echo = FALSE}
fts <- data.frame(feature_id = c("FT02151", "FT02150", 'FT01257', 'FT01805'),
                  confidence_level = c("C", "C", 'D', 'D'))
## Get MS2 spectra:
ms2 <- std_ms2[match(c('FT02151_F07.S0175',
                       'FT02150_F08.S0196', 'FT02150_F07.S0175'),
                     spectraNames(std_ms2))]
ms2$confidence <- c("high",
                    'high', 'high')
```
...
```{r, echo = FALSE}
fts <- data.frame(feature_id = c("FT02227"),
                  confidence_level = c("A"))
## Get MS2 spectra:
ms2 <- std_ms2[match(c('FT02227_F07.S0324', 'FT02227_F07.S0369'),
                     spectraNames(std_ms2))]
ms2$confidence <- c("high")
```
...
```{r, echo = FALSE}
fts <- data.frame(feature_id = c("FT01063"),
...
```{r, echo = FALSE}
fts <- data.frame(feature_id = c("FT01692"),
                  confidence_level = c("A"))
## Get MS2 spectra:
ms2 <- std_ms2[match(c("FT01692_F07.S0712", "FT01692_F08.S0721"),
                     spectraNames(std_ms2))]
...
```{r, echo = FALSE}
fts <- data.frame(feature_id = c("FT1760", "FT1237"),
...
```{r, echo = FALSE}
fts <- data.frame(feature_id = c("FT0837", "FT1421", 'FT0834', 'FT3652'),
                  confidence_level = c("A", "D", 'D', 'D'))
## Get MS2 spectra:
ms2 <- std_ms2[match(c("FT0837_F15.S0203", "FT0837_F15.S0157"),
                     spectraNames(std_ms2))]
ms2$confidence <- c("high", 'high')
```
...
```{r, echo = FALSE}
fts <- data.frame(feature_id = c("FT0564", 'FT2989'),
                  confidence_level = c("B", "D"))
## Get MS2 spectra:
ms2 <- std_ms2[match(c("FT0564_F15.S0790", "FT0564_F15.S0838"),
                     spectraNames(std_ms2))]
ms2$confidence <- c("high", 'high')
```
...
- Reference MS2: FT0918: F16.S0293, F16.S0322, F15.S0370, F16.S0249 
- Reference MS2: FT3856: F15.S0377

```{r, echo = FALSE}
fts <- data.frame(feature_id = c("FT0918", 'FT3856'),
                  confidence_level = c("B", "B"))
## Get MS2 spectra:
ms2 <- std_ms2[match(c("FT0918_F16.S0293", "FT0918_F16.S0322", "FT0918_F15.S0370", 'FT0918_F16.S0249',
                       "FT3856_F15.S0377"),
                     spectraNames(std_ms2))]
...
```{r, echo = FALSE}
fts <- data.frame(feature_id = c("FT1317", "FT0723"),
                  confidence_level = c("D", 'D'))
```
...
| label |   feature  | mzmed | ppm_error | rtmed | target_RT | feature_group |  adduct   | n_ms2 | mean_high | mean_low |
|:-----:|:----------:|:-----:|:---------:|:-----:|:---------:|:-------------:|:---------:|:-----:|:---------:|:--------:|
| **C** | FT1899     | 275   | 16.56     | 268.3 | 265       | FG.001.001    | [M-H]-    | 3     | 15.69     | 7.024    |

```{r, echo = FALSE}
fts <- data.frame(feature_id = c("FT1899"),
                  confidence_level = c("C"))
## Get MS2 spectra:
ms2 <- std_ms2[match(c("FT1899_F15.S0900", "FT1899_F15.S0888"),
                     spectraNames(std_ms2))]
ms2$confidence <- c("high", 'high')
```
