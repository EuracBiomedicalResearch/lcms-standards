...
```{r mix01-water-3-phosphoglyceric-acid-ms2-hmdb-heatmap, fig.path = IMAGE_PATH, fig.width = 8, fig.height = 5, fig.cap = "Similarities of MS2 spectra for selected features against reference spectra for 3-Phoshoglyceric Acid from HMDB."}
...
```{r mix01-water-acetylhistidine-ms2-hmdb-heatmap, fig.path = IMAGE_PATH, fig.width = 8, fig.height = 5, fig.cap = "Similarities of MS2 spectra for selected features against reference spectra for Acetylhistidine from HMDB."}
hmdb_id <- std_dilution$HMDB.code[std_dilution$name == std]
hmdb <- Spectra(cdb, filter = ~ compound_id == hmdb_id)
...
```{r mix01-water-betaine-ms2-hmdb-heatmap, fig.path = IMAGE_PATH, fig.width = 8, fig.height = 5, fig.cap = "Similarities of MS2 spectra for selected features against reference spectra for Betaine from HMDB."}
hmdb_id <- std_dilution$HMDB.code[std_dilution$name == std]
hmdb <- Spectra(cdb, filter = ~ compound_id == hmdb_id)
...
```{r mix01-water-CDP-ms2, fig.path = IMAGE_PATH, fig.cap = "MS2 spectra for features matched to CDP", echo = FALSE}
std_ms2 <- extract_ms2(data_WP, rownames(tmp))
...
```{r mix01-water-creatine-ms2, fig.path = IMAGE_PATH, fig.cap = "MS2 spectra for features matched to creatine", echo = FALSE}
std_ms2 <- extract_ms2(data_WP, rownames(tmp))
...
```{r mix01-water-creatine-ms2-hmdb-heatmap, fig.path = IMAGE_PATH, fig.width = 8, fig.height = 5, fig.cap = "Similarities of MS2 spectra for selected features against reference spectra for creatine from HMDB."}
hmdb_id <- std_dilution$HMDB.code[std_dilution$name == std]
hmdb <- Spectra(cdb, filter = ~ compound_id == hmdb_id)
...
We show a representative EIC for each *feature group* (ordered by
retention time).                                                                                                                                                                                                                                                                                                                                                        
...
```{r mix01-water-dimethylglycine-ms2, fig.path = IMAGE_PATH, fig.cap = "MS2 spectra for features matched to dimethylglycine", echo = FALSE}
std_ms2 <- extract_ms2(data_WP, rownames(tmp))
if (length(std_ms2)) {
    plotSpectra(std_ms2)
} else {
...
```{r mix01-water-glycerol-ms2, fig.path = IMAGE_PATH, fig.cap = "MS2 spectra for features matched to glycerol", echo = FALSE}
std_ms2 <- extract_ms2(data_WP, rownames(tmp))
if (length(std_ms2)) {
    plotSpectra(std_ms2)
} else {
...
```{r mix01-water-L-Glutamic-Acid-ms2, fig.path = IMAGE_PATH, fig.cap = "MS2 spectra for features matched to L-Glutamic Acid.", echo = FALSE}
std_ms2 <- extract_ms2(data_WP, rownames(tmp))
if (length(std_ms2)) {
    plotSpectra(std_ms2)
} else {
...
```{r mix01-water-L-Glutamic-Acid-ms2-hmdb-heatmap, fig.path = IMAGE_PATH, fig.width = 8, fig.height = 5, fig.cap = "Similarities of MS2 spectra for selected features against reference spectra for L-Glutamic Acid from HMDB."}
hmdb_id <- std_dilution$HMDB.code[std_dilution$name == std]
hmdb <- Spectra(cdb, filter = ~ compound_id == hmdb_id)
...
```{r mix01-water-myo-inositol-ms2, fig.path = IMAGE_PATH, fig.cap = "MS2 spectra for features matched to myo-inositol.", echo = FALSE}
std_ms2 <- extract_ms2(data_WP, rownames(tmp))
if (length(std_ms2)) {
    plotSpectra(std_ms2)
} else {
...
```{r mix01-water-myo-inositol-ms2-hmdb-heatmap, fig.path = IMAGE_PATH, fig.width = 8, fig.height = 5, fig.cap = "Similarities of MS2 spectra for selected features against reference spectra for myo-inositol from HMDB."}
hmdb_id <- std_dilution$HMDB.code[std_dilution$name == std]
hmdb <- Spectra(cdb, filter = ~ compound_id == hmdb_id)
...
```{r mix01-water-pyruvic-acid-ms2, fig.path = IMAGE_PATH, fig.cap = "MS2 spectra for features matched to pyruvic-acid", echo = FALSE}
std_ms2 <- extract_ms2(data_WP, rownames(tmp))
if (length(std_ms2)) {
    plotSpectra(std_ms2)
} else {
...
```{r mix01-water-suberic-acid-ms2, fig.path = IMAGE_PATH, fig.cap = "MS2 spectra for features matched to suberic acid.", echo = FALSE}
std_ms2 <- extract_ms2(data_WP, rownames(tmp))
...
```{r mix01-water-suberic-acid-ms2-hmdb-heatmap, fig.path = IMAGE_PATH, fig.width = 8, fig.height = 5, fig.cap = "Similarities of MS2 spectra for selected features against reference spectra for suberic acid from HMDB."}
hmdb_id <- std_dilution$HMDB.code[std_dilution$name == std]
hmdb <- Spectra(cdb, filter = ~ compound_id == hmdb_id)
...
```{r mix01-water-xanthine-ms2, fig.path = IMAGE_PATH, fig.cap = "MS2 spectra for features matched to xanthine", echo = FALSE}
std_ms2 <- extract_ms2(data_WP, rownames(tmp))
...
```{r mix01-water-xanthine-ms2-hmdb-heatmap, fig.path = IMAGE_PATH, fig.width = 8, fig.height = 5, fig.cap = "Similarities of MS2 spectra for selected features against reference spectra for xanthine from HMDB."}
hmdb_id <- std_dilution$HMDB.code[std_dilution$name == std]
hmdb <- Spectra(cdb, filter = ~ compound_id == hmdb_id)
...
```{r mix01-water-neg-bpc, fig.path = IMAGE_PATH, echo = FALSE, caption = "Base peak chromatogram and chromatographic peak density image. Dashed vertical lines represent the expected retention times for the standards as defined in a previous manual analysis.", fig.width = 10, fig.height = 7}
...
abline(v = std_dilution01$RT, lty = 2)
```
<!-- For some samples in lower concentration there are darker bins than the
corresponding high intensity ones -->
...
```{r}
adducts_N <- adducts("negative")[c("[M]-", "[M-H]-", "[M+Cl]-"),
...
pandoc.table(as.data.frame(mD_WN), style = "rmarkdown",
             split.tables = Inf, caption = "Feature to standards matches")
```
...
```{r mix01-water-neg-bpc-matched, fig.path = IMAGE_PATH, echo = FALSE, caption = "Base peak chromatogram with position of features assigned to standards.", fig.width = 10, fig.height = 7}
...
The EICs for the two features are shown below.

![](images/match-standards-mix01/3-Phosphoglyceric Acid/WN_FT0557.png)

![](images/match-standards-mix01/3-Phosphoglyceric Acid/WN_FT0558.png)

<!-- The shape of the curves is similar in both cases but --> 
...
```{r mix01-water-neg-3-phosphoglyceric-acid-ms2, fig.path = IMAGE_PATH, fig.cap = "MS2 spectra for features matched to 3-Phosphoglyceric Acid.", echo = FALSE}
plotSpectra(std_ms2)
```
...
```{r mix01-water-neg-3-phosphoglyceric-acid-ms2-hmdb-heatmap, fig.path = IMAGE_PATH, fig.width = 8, fig.height = 5, fig.cap = "Similarities of MS2 spectra for selected features against reference spectra for 3-Phoshoglyceric Acid from HMDB."}
hmdb_id <- std_dilution$HMDB.code[std_dilution$name == std]
hmdb <- Spectra(cdb, filter = ~ compound_id == hmdb_id)
...
<!-- peaks seem close in m/z but not highlighted --> 
...
Again, we show some EICs.

![](images/match-standards-mix01/Acetylhistidine/WN_FT0620.png)

![](images/match-standards-mix01/Acetylhistidine/WN_FT0621.png)
![](images/match-standards-mix01/Acetylhistidine/WN_FT0623.png)

```{r mix01-water-neg-acetylhistidine-ms2, fig.path = IMAGE_PATH, fig.cap = "MS2 spectra for features matched to acetylhistidine.", echo = FALSE}
std_ms2 <- extract_ms2(data_WN, rownames(tmp))
...
```{r mix01-water-neg-acetylhistidine-ms2-hmdb-heatmap, fig.path = IMAGE_PATH, fig.width = 8, fig.height = 5, fig.cap = "Similarities of MS2 spectra for selected features against reference spectra for Acetylhistidine from HMDB."}
hmdb_id <- std_dilution$HMDB.code[std_dilution$name == std]
hmdb <- Spectra(cdb, filter = ~ compound_id == hmdb_id)
...
![](images/match-standards-mix01/Betaine/WN_FT0386.png)

![](images/match-standards-mix01/Betaine/WN_FT0323.png)
<!--
The signal from the first feature above looks OK while the others are rather
noisy.
-->
...
```{r mix01-water-neg-betaine-ms2, fig.path = IMAGE_PATH, fig.cap = "MS2 spectra for features matched to betaine.", echo = FALSE}
std_ms2 <- extract_ms2(data_WN, rownames(tmp))
...
Again, we show some EICs for the features above.


![](images/match-standards-mix01/CDP/WN_FT2130.png)

![](images/match-standards-mix01/CDP/WN_FT2131.png)

![](images/match-standards-mix01/CDP/WN_FT2133.png)

![](images/match-standards-mix01/CDP/WN_FT2132.png)
We next extract the MS2 spectra for all these features and match them against
the reference spectra for `r std` from HMDB.

```{r mix01-water-neg-CDP-ms2, fig.path = IMAGE_PATH, fig.cap = "MS2 spectra for features matched to CDP", echo = FALSE}
std_ms2 <- extract_ms2(data_WN, rownames(tmp))
...
Again, we show some EICs for the two features above.

![](images/match-standards-mix01/Creatine/WN_FT0189.png)

![](images/match-standards-mix01/Creatine/WN_FT0180.png)

We next extract the MS2 spectra for all these features and match them against
the reference spectra for `r std` from HMDB.

```{r mix01-water-neg-creatine-ms2, fig.path = IMAGE_PATH, fig.cap = "MS2 spectra for features matched to creatine", echo = FALSE}
std_ms2 <- extract_ms2(data_WN, rownames(tmp))
...
We show a representative EIC for each *feature group* (ordered by
retention time).                                                                                                                                                                                                                                                                                                                                                        
![](images/match-standards-mix01/Dimethylglycine/WN_FT0052.png)
We next extract the MS2 spectra for all these features and match them against
the reference spectra for `r std` from HMDB.

```{r mix01-water-neg-dimethylglycine-ms2, fig.path = IMAGE_PATH, fig.cap = "MS2 spectra for features matched to dimethylglycine", echo = FALSE}
std_ms2 <- extract_ms2(data_WN, rownames(tmp))
if (length(std_ms2)) {
    plotSpectra(std_ms2)
} else {
...
![](images/match-standards-mix01/Glycerol/WN_FT0234.png)
We next extract the MS2 spectra for all these features and match them against
the reference spectra for `r std` from HMDB.

```{r mix01-water-neg-glycerol-ms2, fig.path = IMAGE_PATH, fig.cap = "MS2 spectra for features matched to glycerol", echo = FALSE}
std_ms2 <- extract_ms2(data_WN, rownames(tmp))
if (length(std_ms2)) {
    plotSpectra(std_ms2)
} else {
...
![](images/match-standards-mix01/L-Glutamic Acid/WN_FT0270.png)

![](images/match-standards-mix01/L-Glutamic Acid/WN_FT0288.png)

![](images/match-standards-mix01/L-Glutamic Acid/WN_FT0271.png)

We next extract the MS2 spectra for all these features and match them against
the reference spectra for `r std` from HMDB.

```{r mix01-water-neg-L-Glutamic-Acid-ms2, fig.path = IMAGE_PATH, fig.cap = "MS2 spectra for features matched to L-Glutamic Acid.", echo = FALSE}
std_ms2 <- extract_ms2(data_WN, rownames(tmp))
if (length(std_ms2)) {
    plotSpectra(std_ms2)
} else {
...
```{r mix01-water-neg-L-Glutamic-Acid-ms2-hmdb-heatmap, fig.path = IMAGE_PATH, fig.width = 8, fig.height = 5, fig.cap = "Similarities of MS2 spectra for selected features against reference spectra for L-Glutamic Acid from HMDB."}
hmdb_id <- std_dilution$HMDB.code[std_dilution$name == std]
hmdb <- Spectra(cdb, filter = ~ compound_id == hmdb_id)
...
![](images/match-standards-mix01/Myo-Inositol/WN_FT0813.png)

We next extract the MS2 spectra for all these features and match them against
the reference spectra for `r std` from HMDB.

```{r mix01-water-neg-myo-inositol-ms2, fig.path = IMAGE_PATH, fig.cap = "MS2 spectra for features matched to myo-inositol.", echo = FALSE}
std_ms2 <- extract_ms2(data_WN, rownames(tmp))
if (length(std_ms2)) {
    plotSpectra(std_ms2)
} else {
...
```{r mix01-water-neg-myo-inositol-ms2-hmdb-heatmap, fig.path = IMAGE_PATH, fig.width = 8, fig.height = 5, fig.cap = "Similarities of MS2 spectra for selected features against reference spectra for myo-inositol from HMDB."}
hmdb_id <- std_dilution$HMDB.code[std_dilution$name == std]
hmdb <- Spectra(cdb, filter = ~ compound_id == hmdb_id)
...
![](images/match-standards-mix01/Pyruvic Acid/WN_FT0023.png)
![](images/match-standards-mix01/Pyruvic Acid/WN_FT0024.png)

We next look if there are MS2 spectra associated to the above features.

```{r mix01-water-neg-pyruvic-acid-ms2, fig.path = IMAGE_PATH, fig.cap = "MS2 spectra for features matched to pyruvic-acid", echo = FALSE}
std_ms2 <- extract_ms2(data_WN, rownames(tmp))
if (length(std_ms2)) {
    plotSpectra(std_ms2)
} else {
...
IWe match the above MS2 spectrum against all spectra from HMDB.
...
Again, we show some of the obtained EICs.

![](images/match-standards-mix01/Suberic Acid/WN_FT0428.png)

![](images/match-standards-mix01/Suberic Acid/WN_FT0430.png)

![](images/match-standards-mix01/Suberic Acid/WN_FT0429.png)

We next extract the MS2 spectra for all these features and match them against
the reference spectra for `r std` from HMDB.

```{r mix01-water-neg-suberic-acid-ms2, fig.path = IMAGE_PATH, fig.cap = "MS2 spectra for features matched to suberic acid.", echo = FALSE}
std_ms2 <- extract_ms2(data_WN, rownames(tmp))
...
```{r mix01-water-neg-suberic-acid-ms2-hmdb-heatmap, fig.path = IMAGE_PATH, fig.width = 8, fig.height = 5, fig.cap = "Similarities of MS2 spectra for selected features against reference spectra for suberic acid from HMDB."}
hmdb_id <- std_dilution$HMDB.code[std_dilution$name == std]
hmdb <- Spectra(cdb, filter = ~ compound_id == hmdb_id)
...
![](images/match-standards-mix01/Uric Acid/WN_FT0397.png)
![](images/match-standards-mix01/Uric Acid/WN_FT0398.png)

We next look if there are MS2 spectra associated to the above features.

```{r mix01-water-neg-uric-acid-ms2, fig.path = IMAGE_PATH, fig.cap = "MS2 spectra for features matched to pyruvic-acid", echo = FALSE}
std_ms2 <- extract_ms2(data_WN, rownames(tmp))
if (length(std_ms2)) {
    plotSpectra(std_ms2)
} else {
...
![](images/match-standards-mix01/Xanthine/WN_FT0316.png)

![](images/match-standards-mix01/Xanthine/WN_FT0321.png)
![](images/match-standards-mix01/Xanthine/WN_FT0315.png)

We next extract the MS2 spectra for all these features and match them against
the reference spectra for `r std` from HMDB.

```{r mix01-water-neg-xanthine-ms2, fig.path = IMAGE_PATH, fig.cap = "MS2 spectra for features matched to xanthine", echo = FALSE}
std_ms2 <- extract_ms2(data_WN, rownames(tmp))
