...
sign_f <- cbind(sign_f_id, fD[sign_f_id,c("mzmed", "rtmed")], 
               pvalues[sign_f_id,], 
               feature_group = fD[sign_f_id, c("feature_group")])
sign_f <- sign_f[order(sign_f[, "rtmed" ]),]
sign_f <- sign_f[order(sign_f[, "feature_group" ]),]
rtmed_f=split(sign_f$rtmed, sign_f$feature_group) %>%
vapply(FUN = mean, numeric(1))
rtmed_f=rep(rtmed_f, unname(table(sign_f$feature_group)))
sign_f <- cbind(sign_f,rtmed_f)
sign_f <- sign_f[order(sign_f[, "rtmed_f" ]),]
...
{
  addnames <- colnames(matches_mat)[which(!is.na(matches_mat[ft, ]))]
  res <- cbind(addnames, 
...
for (i in 1:nrow(sign_f))
{
  ...
  if(nrow(addcts)>0)
    matchings=rbind(matchings,cbind(id=sign_f$sign_f_id[i],
                                    group=sign_f$feature_group[i],addcts)) 
}
```

Matchings per group feature
```{r}
matchings_groups <- lapply(unique(matchings$group), function(x) matchings[which(matchings$group==x), c("addnames","name","formula")])
names(matchings_groups)=unique(matchings$group)
```

Plot attempts: still to complete

```{r}
pdf(paste0(IMAGE_PATH,"matchings.pdf"), width=20,height=10)
par(mar=c(5.1, 10, 4.1, 2.1))
v=as.vector(matches_mat)
x=rep(1:nrow(matches_mat), ncol(matches_mat))
y=rep(1:ncol(matches_mat), each=nrow(matches_mat))
x[is.na(v)]=NA
y[is.na(v)]=NA
plot(x,y, ylim=c(0,40), col=rainbow(15)[v], xlab="features", ylab= "adducts", xaxt="n", yaxt="n", pch = 16)
axis(side=1, at = 1:nrow(matches_mat), labels= FALSE)
axis(side=2, at = 1:ncol(matches_mat), labels= FALSE)
text(x, y, labels=frmls[v], cex= 0.7, pos=3, srt =90)
text(par("usr")[1], 1:ncol(matches_mat), labels = colnames(matches_mat), srt = 0, pos = 2, xpd = TRUE)
abline(h=1:ncol(matches_mat),col="grey85")
dev.off()
```
