...
```{r}
fD_WP <- featureDefinitions(data_WP)
fD_WN <- featureDefinitions(data_WN)
fD_SP <- featureDefinitions(data_SP)
fD_SN <- featureDefinitions(data_SN)
#' feature intensity matrices
fmat_WP <- featureValues(data_WP, value = "into", method = "sum")
fmat_WN <- featureValues(data_WN, value = "into", method = "sum")
fmat_SP <- featureValues(data_SP, value = "into", method = "sum")
fmat_SN <- featureValues(data_SN, value = "into", method = "sum")
```

```{r}
#' Percentage of missing values before filling
sum(is.na(fmat_WP)) / length(fmat_WP) # positive polarity
sum(is.na(fmat_WN)) / length(fmat_WN) # negative polarity
sum(is.na(fmat_SP)) / length(fmat_SP) # positive polarity
sum(is.na(fmat_SN)) / length(fmat_SN) # negative polarity
```
...
save(data_WP, data_WN, data_SP, data_SN, file = paste0(RDATA_PATH, "data_postfilling.RData"))
```
