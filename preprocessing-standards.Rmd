...
```{r}
#t.test(x=fmat_pos_wat[1,c(1,3,6)]-fmat_pos_wat[1,c(2,4,5)], mu = 0, alternative= "less")
# H0: high<=low vs H1: H0^c
high <- grep("^High",colnames(fmat_pos_watl2)) #c(1,3,6)
low <- grep("^Low",colnames(fmat_pos_watl2)) #c(2,4,5)
# Compute the p-values
#pvalues_two <- apply(fmat_pos_watl2, 1, function(x) t.test(x[high] - x[low], mu = 0)$p.value)
