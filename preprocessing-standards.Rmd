...
We subset the data according to matrix (and polarity) considering FS mode only 
data

```{r}
data_WP <- filterFile(data_pos, file = which(data_pos$matrix_pol == "Water_POS" 
                                   & data_pos$mode == "FS"), 
                      keepFeatures = TRUE)
data_WN <- filterFile(data_neg, file = which(data_neg$matrix_pol == "Water_NEG" 
                                   & data_neg$mode == "FS"), 
                      keepFeatures = TRUE)
data_SP <- filterFile(data_pos, file = which(data_pos$matrix_pol == "Serum_POS" 
                                   & data_pos$mode == "FS"), 
                      keepFeatures = TRUE)
data_SN <- filterFile(data_neg, file = which(data_neg$matrix_pol == "Serum_NEG" 
                                   & data_neg$mode == "FS"), 
                      keepFeatures = TRUE)
```
...
plotfeatures <- function(features, data_subs, mp)
{
  ...
  mzMLnames <- data_subs$mzML
  low <- grep("Low", mzMLnames)
  ...
  for (i in 1:nrow(f_chrs)) {
    ...
    chr_obj <- f_chrs[i, match(mzMLnames, f_chrs$mzML)]
...
```{r feature_groups_plots_WP, echo = FALSE}
...
plot_groups_of_features(sign_f_WP$sign_f_id, sign_f_WP$feature_group,
                       data_WP, paste0(IMAGE_PATH, "feature_groups/"))
```
...
```{r feature_groups_plots_WN, echo = FALSE}
## order feature groups by retention time and plot each.
...
```{r}
get_matchings <- function(info_stds, sign_f, pol)
{
...
```{r features_by_standards, echo = FALSE}
## order feature groups by retention time and plot each.
plot_groups_of_features(matchings_WP$feature_id, matchings_WP$name,
                       data_WP, paste0(IMAGE_PATH, "features_by_standards/"))
```
...
ft_WP <- cbind(as.data.frame(sign_f_WP), pk = paste0(sign_f_WP$sign_f_id, "_", mix))
ft_WN <- cbind(as.data.frame(sign_f_WN), pk = paste0(sign_f_WN$sign_f_id, "_", mix))
ft_SP <- cbind(as.data.frame(sign_f_SP), pk = paste0(sign_f_SP$sign_f_id, "_", mix))
ft_SN <- cbind(as.data.frame(sign_f_SN), pk = paste0(sign_f_SN$sign_f_id, "_", mix))
ft_to_comp_WP <- cbind(compound_pk = paste0(matchings_WP$name, "_", mix),
                       feature_pk = paste0(matchings_WP$feature_id, "_", mix),
                       adduct = matchings_WP$adduct, matrix_pol = "Water_POS")
ft_to_comp_WN <- cbind(compound_pk = paste0(matchings_WN$name, "_", mix),
                       feature_pk = paste0(matchings_WN$feature_id, "_", mix),
                       adduct = matchings_WN$adduct, matrix_pol = "Water_NEG")
ft_to_comp_SP <- cbind(compound_pk = paste0(matchings_SP$name, "_", mix),
                       feature_pk = paste0(matchings_SP$feature_id, "_", mix),
                       adduct = matchings_SP$adduct, matrix_pol = "Serum_POS")
ft_to_comp_SN <- cbind(compound_pk = paste0(matchings_SN$name, "_", mix),
                       feature_pk = paste0(matchings_SN$feature_id, "_", mix),
...
pandoc.table(stab2, style = "rmarkdown", split.table = "Inf",
             caption = paste0("Matching of theoretical adducts of standards ",
                              ...
                              "difference of abundances."))
```
# Get for each feature the assigned standard
