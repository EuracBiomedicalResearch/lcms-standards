...
```{r}
ftype <- paste0("Mix", ifelse(mix<10,paste0(0,mix),mix)) 
std_serum_files01 <- std_serum_files[which(std_serum_files$type == ftype), ]
```
...
```{r refinement, echo = FALSE, eval = file.exists(paste0(RDATA_PATH, "data_refined.RData"))}
...
```{r}
keep_features <- rowSums(is.na(fmat_pos[, which(data_pos$matrix_pol == "Water_POS" 
                                 & data_pos$mode == "FS")])) <= 3
fmat_pos_wat <- fmat_pos_f[ keep_features, which(data_pos$matrix_pol == "Water_POS" 
                                 & data_pos$mode == "FS")]

# fmat_pos_wat <- fmat_pos_f[, which(data_pos$matrix_pol == "Water_POS" 
#                                  & data_pos$mode == "FS")]
...
```{r}
se_pos <- quantify(data_pos, method = "sum", value = "into")
se <- se_pos[sign_f$sign_f_id, which(data_pos$matrix_pol == "Water_POS" & 
                                       data_pos$mode == "FS")]
se <- groupFeatures(se, param = SimilarRtimeParam(2), rtime = "rtmed")
...
NEGATIVE
```{r}
keep_features <- rowSums(is.na(fmat_neg[, which(data_neg$matrix_pol == "Water_NEG" 
                                 & data_neg$mode == "FS")])) <= 3
fmat_neg_wat <- fmat_neg_f[ keep_features, which(data_neg$matrix_pol == "Water_NEG" 
                                 & data_pos$mode == "FS")]

# fmat_neg_wat <- fmat_neg_f[, which(data_neg$matrix_pol == "Water_NEG" 
#                                  & data_neg$mode == "FS")]
  
# Exclude rows with NA
fmat_neg_wat <- na.omit(fmat_neg_wat)
```

and log2 transform the abundances (and keep only rows without missing values)

```{r}
fmat_neg_watl2 <- log2(fmat_neg_wat)
```
...
```{r}
# H0: high==low vs H1: H0^c
high <- grep("^High", colnames(fmat_neg_watl2)) #c(1,3,6)
low <- grep("^Low", colnames(fmat_neg_watl2)) #c(2,4,5)

pvalues <- apply(fmat_neg_watl2, 1, function(x) {
    res <- t.test(x[high], x[low], mu = 0)
    c(pvalue = res$p.value,
      M = unname(res$estimate[1] - res$estimate[2]))
})
pvalues <- t(pvalues)
# Can data be assumed normal?
# hist(apply(fmat_neg_watl2, 1, function(x) shapiro.test(x[high]-x[low])$p.value))
```
...
```{r, echo = FALSE, results = "asis"}
#sign_f_indexes <- which(pvalues[, "pvalue"] < 0.05 & pvalues[, "M"] > 3)
sign_f_id <- rownames(fmat_neg_wat)[which(pvalues[, "pvalue"] < 0.05 & pvalues[, "M"] > 3)]
sign_f <- cbind(sign_f_id, fD_neg[sign_f_id,c("mzmed", "rtmed")], 
               pvalues[sign_f_id,])
sign_f <- sign_f[order(sign_f[, "rtmed" ], decreasing = FALSE),]
pandoc.table(data.frame(sign_f), style = "rmarkdown", caption = "Significant features")
```
...
```{r}
f_chrs <- featureChromatograms(data_neg, expandRt = 2, 
                               features = sign_f_id, filled = TRUE)
#plot(f_chrs[1, match(colnames(fmat_neg_wat), f_chrs$mzML)])
```
...
{
png(filename = paste0(IMAGE_PATH, "selectedfeatures/feature", i, ".png"), width=700, height=500)
chr_obj=f_chrs[i, match(colnames(fmat_neg_wat), f_chrs$mzML)]
plot(chr_obj, col = col_hl, 
     peakBg = paste0(col_hl[chromPeaks(chr_obj)[, "sample"]], 40))
legend("topright", col = brewer.pal(5, "Set1")[c(3,4)],
       legend = c("high","low"), lwd = 1, cex = 1)
...
```{r}
se_neg <- quantify(data_neg, method = "sum", value = "into")
se <- se_neg[sign_f$sign_f_id, which(data_neg$matrix_pol == "Water_neg" & 
                                       data_neg$mode == "FS")]
se <- groupFeatures(se, param = SimilarRtimeParam(2), rtime = "rtmed")
table(featureGroups(se))
```
